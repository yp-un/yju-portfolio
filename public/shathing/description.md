...
현재는 Next.js 프론트엔드와 React Native 앱에서 API 타입, 도메인 유틸, 디자인 토큰처럼 공통으로 사용하는 로직을 각 프로젝트에서 별도로 관리하고 있다. 이후에는 공통 패키지로 분리해 웹과 앱이 같은 타입과 비즈니스 규칙을 공유하도록 개선할 수 있다. 이를 통해 기능 변경 시 중복 수정 범위를 줄이고, 디자인 토큰도 단일 기준으로 관리해 플랫폼 간 UI 일관성을 더 안정적으로 유지할 수 있다.
